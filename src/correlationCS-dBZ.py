...
if __name__ == "__main__":
    ...
    for date in existDateOpt:
        ...
        condition = np.logical_and(csData != 0, thdData != 0)
        x = csData[condition]
        y = dBZData[condition]
        c = thdData[condition]
    ...
    title("Correlation of {X} and {Y}".format(X="CS", Y="dBZ"), fontsize=25, y=1.05)
    ...
    xlabel("Column of Snow", fontsize=15)
    ylabel("dBZ", fontsize=15)
    xticks(fontsize=15)
    yticks(fontsize=15)
    #ylim(bottom=-5)
    cbar = colorbar(extend="max")
    cbar.set_label("Freq of CG")
    savefig("CG{}_cs-dBZ.jpg".format(thdHour))
    clf()
