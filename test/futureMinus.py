...
if __name__ == "__main__":
    ...
    if caseType == "mid21":
        ...
        data = endChange
    amplitude = int(max(np.nanmax(data), np.abs(np.nanmin(data))))
    cmap = cm.get_cmap("seismic", lut=amplitude*2)
    FREQ = ax.pcolormesh(xlon, xlat, data, vmin=-amplitude, vmax=amplitude, cmap=cmap)
    cbar = plt.colorbar(FREQ)
    cbar.ax.tick_params(labelsize=15)
    plt.title("Difference between {} and hist".format(caseType), fontsize=20, y=1.05)
    ...
    plt.savefig("{}Minus-{}.jpg".format(caseType, methodName), dpi=250)
    plt.clf()
