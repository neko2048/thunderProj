...
if __name__ == "__main__":
    ...
    percentileMap = np.load("./percentileMap-{}.npy".format(methodName))
    ...
    if caseType == "mid21":
        data = np.where(percentileMap >= 90, midChange, np.nan)
        FREQ = ax.pcolormesh(xlon, xlat, data, vmin=-100, vmax=100, cmap=cmap)
    elif caseType == "end21":
        data = np.where(percentileMap >= 90, endChange, np.nan)
        FREQ = ax.pcolormesh(xlon, xlat, data, vmin=-100, vmax=100, cmap=cmap)
    ...
    plt.savefig("{}Change90-{}.jpg".format(caseType, methodName), dpi=250)
    plt.clf()
